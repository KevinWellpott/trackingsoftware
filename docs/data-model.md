...
- **Arbeitsliste** = `/termine?view=liste`, die zentrale Arbeitsfläche seit dem Rückbau. Geschnitten wird über drei Reiter-Werte in `?zeit=`: **Zu tun** (Vorgabe, = Arbeitsmenge **plus** die Termine mit offener Erinnerung, `istZuTun()`) · **Termin steht** (URL-Wert weiterhin `verlegt`) · **Alle**. Jeder Ausschnitt trägt die Zahl der Zeilen, die er zeigt — gebildet aus derselben Menge, die die Liste rendert (Suche und Personenachse also schon eingerechnet). Die ersten beiden sind **nicht disjunkt**: Ein Termin, der morgen ansteht und heute angekündigt werden muss, steht in beiden; die drei Zahlen addieren sich deshalb nicht zu „Alle" (das taten sie auch vorher nicht, weil die abgeschlossenen Vorgänge nur unter „Alle" stehen). Drei Knöpfe je Zeile, alle ohne Seitenwechsel: **Genervt** (stempelt, ohne Rückfrage — folgenlos), **Termin** (Datum-Zeit-Feld; danach `verlegt` und raus aus der Arbeitsmenge), **Tot** (mit Rückfrage, weil es die Zeile beendet). Genau zwei Abgangskriterien, nichts sonst. Personenachse `?wer=`: `mein` (Vorgabe, `effective_user_id ?? user.id` nach `personOf()`) oder `alle` — Letzteres nur für einen Owner mit Team-Sicht, sonst wäre der Schalter eine Lüge über zugeschnittene Daten.
- **Abgeleiteter Zustand** = die EINE Aussage je Zeile der Arbeitsliste (`terminZustand()`, `src/lib/dranRegel.ts`): `verlegt` · `offen` · `show` · `no_show` · `qualifiziert` · `closing_gelegt` · `nicht_qualifiziert` · `tot` · `close` · `kein_close`. **`verlegt` heißt auf dem Bildschirm „Termin steht"** — der Schlüssel beschreibt die Datenlage korrekt und steckt in geteilten Links (`?zeit=verlegt`), das Wort „Verlegt" ist gegangen, weil es nach „wurde verschoben" klang und der Ausschnitt tatsächlich JEDEN Termin mit Datum in der Zukunft enthält. Ausschnitt und Status-Pill tragen dasselbe Wort; zwei Wörter für dieselbe Aussage in derselben Zeile waren die Verwechslung, die dahin geführt hat. **Steht in keiner Spalte und wird nirgends geschrieben** — er entsteht bei jedem Rendern aus `status`, `show_status`, dem Termin, `cancelled_at` und `revived_at`. Siehe die Warnung „Die abgeleiteten Zustände" unten; sie ist die wichtigste Stelle dieses Abschnitts.
- **Termin-Erinnerung** = die zwei festen Zeitpunkte vor jedem Termin, aus dem Termin gerechnet: **1 Tag vorher** und **1 Stunde vorher**, für Erstgespräch und Closing gleich (`offeneErinnerung()`, `src/lib/dranRegel.ts`). Eine Erinnerung ist **offen**, wenn ihr Zeitpunkt erreicht ist **und** der Nachfass-Stempel älter ist als dieser Zeitpunkt (oder fehlt); die Zeile leuchtet dann gold, obwohl ein Termin steht. **Ein Klick auf „Genervt" schließt genau die fällige Erinnerung** — der Stempel wird gegen die MARKE geprüft, nicht gegen den Kalendertag, und weil die Marken hintereinander liegen (die Vortags-Marke 23–25 h vor dem Termin, die Stunden-Marke 1 h davor), geht die zweite danach von allein wieder auf. **Es gibt keinen Überfällig-Zustand**: Eine Marke, die beim Buchen schon vorbei war (kurzfristiger Termin), ist fällig, aber kein Versäumnis — deshalb wird sie nie über `isOverdue()` gelesen und nie rot (§6). Keine Tabelle, keine Stufen, keine Vorlagen, keine Kanäle, keine Einstellung — zwei Konstanten (`ERINNERUNG_VORTAG_TAGE`, `ERINNERUNG_KURZ_MINUTEN`), wie `CONTACT_GAP_WARN_DAYS` es vorgemacht hat. Nicht zu verwechseln mit der gefallenen **Kaskade** (unten): Die hatte neun Arten, konfigurierbare Stufen, Texte und eine eigene Tabelle.
