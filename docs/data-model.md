...
**0041 ist eingespielt (12. September 2026).** Sie entfernt die drei Pflichtfeld-Trigger aus 0035 und
...
Die ZWEI FESTEN ERINNERUNGEN vor jedem Termin (quer zum Zustand, §1 unten):
  1 Tag vorher (gleiche Uhrzeit am Vortag) und 1 Stunde vorher — für Erstgespräch
  und Closing gleich. Ist eine davon erreicht und der Nachfass-Stempel älter als
  sie, gehört die Zeile trotz stehendem Termin auf die Arbeitsliste und leuchtet.
  Keine Tabelle, keine Stufen, keine Vorlagen, keine Kanäle, keine Einstellung —
  zwei Konstanten in src/lib/dranRegel.ts.
...
HISTORISCH (bis 10.09.2026, Code gefallen — die Tabellen stehen noch, §3):
  ...
  NICHT wiederbelebt durch die zwei festen Erinnerungen oben: Die rechnen zwei
  Zeitpunkte aus dem Termin und schreiben nichts — keine Tabelle, keine Stufen,
  keine Texte, keine Kanäle, keine Konfiguration.
```
...
**Die zwei festen Erinnerungen sind KEIN dritter Mechanismus** — sie sind eine zweite Tür in
dieselbe Arbeitsliste. Sie erzeugen keine Zeile, keine Aufgabe und keine Zeile in einer
Tabelle; sie beantworten für eine Zeile, die ohnehin da ist, zusätzlich die Frage „muss ich
sie HEUTE anfassen, obwohl ein Termin steht?". Dass sie trotzdem eigens genannt werden
müssen, liegt an der Körnung: Die Arbeitsliste rechnet auf Berliner Kalendertagen, diese
beiden Marken auf die **Minute** (§6). Sie sind damit neben dem Telefon-Rückruf die zweite
Stelle der ganzen Software, an der eine Uhrzeit über „du bist dran" entscheidet — und die
einzige, an der die Uhrzeit nicht verabredet, sondern aus dem Termin gerechnet ist.
...
- **Arbeitsliste** = `/termine?view=liste`, die zentrale Arbeitsfläche seit dem Rückbau. Geschnitten wird über **vier** Werte in `?zeit=`: **Zu tun** (Vorgabe, = Arbeitsmenge **plus** die Termine mit offener Erinnerung, `istZuTun()`) · **Erinnerung Setting** (`erinnerung_setting`) · **Erinnerung Closing** (`erinnerung_closing`) · **Alle**. Jeder Ausschnitt trägt die Zahl der Zeilen, die er zeigt — gebildet aus derselben Menge, die die Liste rendert (Suche und Personenachse also schon eingerechnet). Sie sind **nicht disjunkt**: Ein Termin, der morgen ansteht und heute angekündigt werden muss, steht in „Zu tun" UND in seiner Erinnerungs-Ansicht; die vier Zahlen addieren sich deshalb nicht zu „Alle" (das taten sie auch vorher nicht, weil die abgeschlossenen Vorgänge nur unter „Alle" stehen). Drei Knöpfe je Zeile, alle ohne Seitenwechsel: **Genervt** (stempelt, ohne Rückfrage — folgenlos), **Termin** (Datum-Zeit-Feld; danach `verlegt` und raus aus der Arbeitsmenge), **Tot** (mit Rückfrage, weil es die Zeile beendet). Sie hängen an `istBearbeitbar()` = Arbeitsmenge **plus jeder stehende Termin plus `nicht_qualifiziert`**, also ausschließlich am Zustand. Die dritte Hälfte kam am 15. September 2026 dazu („bei Nicht qualifiziert fehlen Genervt Termin Tot"): Ein disqualifiziertes Erstgespräch ist kein totes Ende, ein zweiter Anlauf ist dort der Normalfall. Es leuchtet trotzdem **nicht** Gold und steht **nicht** in „Zu tun" — die Knöpfe sind eine Möglichkeit, keine tägliche Pflicht. „Termin" schreibt dort nicht bloß ein Datum, sondern macht das Erstgespräch wieder auf (`reopenUnqualifiedSetting`: `status` → `offen`, `follow_up_due` und Recycling-Datum geräumt, `show_status` und Disqualifikationsgrund bleiben als Historie); ein bloßes Datum änderte nichts, weil das Ergebnis im abgeleiteten Zustand jedes Datum schlägt. Die Knöpfe hängen bewusst **nicht** an `istZuTun()`: Das war der Fehler, wegen dem in der Erinnerungs-Ansicht durchgehend ein Strich in der Spalte Aktion stand, und es kippte zusätzlich mitten am Tag um (wer morgens stempelte, verlor die Knöpfe). Personenachse `?wer=`: `mein` (Vorgabe, `effective_user_id ?? user.id` nach `erinnererOf()` — wer den Termin gelegt hat, auch bei No-Show und „Nicht qualifiziert"; nur ein erschienener Lead ohne Ergebnis geht an „Durchgeführt von", sofern eingetragen, §2) oder `alle` — Letzteres nur für einen Owner mit Team-Sicht, sonst wäre der Schalter eine Lüge über zugeschnittene Daten.
- **Erinnerungs-Ansicht** = die beiden Ausschnitte `erinnerung_setting` / `erinnerung_closing`. Sie zeigen **alle stehenden Termine ihrer Art** (`zustand === 'verlegt'`), nicht nur die mit gerade offener Erinnerung: Die Frage dort ist nicht „was muss ich jetzt tun" (das steht in „Zu tun"), sondern **„bekommt jeder seine zwei Erinnerungen"** — und die lässt sich nur an der vollständigen Liste beantworten. Die vierte Spalte trägt deshalb nicht den Status, sondern **die zwei Marken als abzählbare Chips** (`1.` `2.`, Stand je Stufe); der Status-Pill stünde dort in jeder Zeile gleich („Termin steht" ist die Bedingung der Ansicht) und wäre reines Rauschen. **Zwei Ansichten statt einer**, weil Erstgespräch und Closing zwei Arbeitsvorgänge mit verschiedenen Gesprächspartnern sind — gemischt sucht man seine fünf Closings zwischen vierzig Settings. Die Zuordnung Ausschnitt → Termin-Art steht an genau einer Stelle: `erinnerungsArt()` (`src/components/termine/viewState.ts`); sie entscheidet Zeilen, Spalte und Leerzustand zugleich. **Historie:** Bis zum 13. September 2026 gab es hier EINEN Ausschnitt `verlegt`, beschriftet „Termin steht". Alte Links mit `?zeit=verlegt` fallen auf `zu_tun` zurück — ein Schlüssel, der beide Termin-Arten meinte, lässt sich nicht ohne Verlust auf eine Hälfte abbilden.
- **Abgeleiteter Zustand** = die EINE Aussage je Zeile der Arbeitsliste (`terminZustand()`, `src/lib/dranRegel.ts`): `verlegt` · `offen` · `show` · `no_show` · `qualifiziert` · `closing_gelegt` · `nicht_qualifiziert` · `tot` · `close` · `kein_close`. **`verlegt` heißt auf dem Bildschirm „Termin steht"** — der Schlüssel beschreibt die Datenlage korrekt, das Wort „Verlegt" ist gegangen, weil es nach „wurde verschoben" klang, während tatsächlich JEDER Termin mit Datum in der Zukunft darunter fällt. **Zustand und Ausschnitt sind seit der Aufteilung zweierlei** und tragen bewusst verschiedene Wörter: Der Zustand benennt die Datenlage EINER Zeile („ein Termin steht"), der Ausschnitt die AUFGABE einer Ansicht („hier wird erinnert"). Eine Runde lang waren sie identisch, weil es die Ansicht nur als Filter auf den Zustand gab. **Steht in keiner Spalte und wird nirgends geschrieben** — er entsteht bei jedem Rendern aus `status`, `show_status`, dem Termin, `cancelled_at` und `revived_at`. Siehe die Warnung „Die abgeleiteten Zustände" unten; sie ist die wichtigste Stelle dieses Abschnitts.
- **Termin-Erinnerung** = die zwei festen Zeitpunkte vor jedem Termin, aus dem Termin gerechnet: **1 Tag vorher** und **1 Stunde vorher**, für Erstgespräch und Closing gleich (`erinnerungsStand()`, `src/lib/dranRegel.ts`). Jede Stufe trägt eine feste **Nummer** (1 = Vortag, 2 = Stunde) und einen von genau **drei Ständen**: `ausstehend` · `offen` · `erledigt`. **Offen** heißt: Zeitpunkt erreicht **und** Nachfass-Stempel älter als dieser Zeitpunkt (oder fehlend); die Zeile leuchtet dann gold, obwohl ein Termin steht. `offeneErinnerung()` ist nur noch die Frage „welche ist gerade dran" an dieses Ergebnis (die spätere zuerst) — **keine zweite Rechnung**, damit die beiden Formulierungen nicht auseinanderlaufen können. **Ein Klick auf „Genervt" schließt genau die fällige Erinnerung** — der Stempel wird gegen die MARKE geprüft, nicht gegen den Kalendertag, und weil die Marken hintereinander liegen (die Vortags-Marke 23–25 h vor dem Termin, die Stunden-Marke 1 h davor), geht die zweite danach von allein wieder auf. Ein Kontakt **vor** der ersten Marke verbraucht entsprechend keine der beiden. **Es gibt keinen vierten Stand „versäumt"**: Eine Marke, die beim Buchen schon vorbei war (kurzfristiger Termin), ist fällig, aber kein Versäumnis — deshalb wird sie nie über `isOverdue()` gelesen und nie rot (§6); sie bleibt auch Stufe 1 und wird nicht übersprungen. Beide Stufen enden **mit dem Termin** (`nowMs >= termin` → gar keine Stufen), obwohl der Zustand tagesgenau noch `verlegt` bleibt. Keine Tabelle, keine Vorlagen, keine Kanäle, keine Einstellung — zwei Konstanten (`ERINNERUNG_VORTAG_TAGE`, `ERINNERUNG_KURZ_MINUTEN`), wie `CONTACT_GAP_WARN_DAYS` es vorgemacht hat. Nicht zu verwechseln mit der gefallenen **Kaskade** (unten): Die hatte neun Arten, konfigurierbare Stufen, Texte und eine eigene Tabelle.
- **Gold-Regel** = „du bist dran". Eine Zeile leuchtet gold, wenn sie in der Arbeitsmenge liegt **und** heute noch niemand an ihr war — oder wenn eine ihrer zwei Termin-Erinnerungen offen ist. Die Regel steht EINMAL (`src/lib/dranRegel.ts`) und wird von der LinkedIn-Liste (`istKontaktDran`) und der Terminliste (`istTerminDran`) geteilt — samt Farbton (`DRAN_TONE`, die vorhandenen Warn-Tokens, keine neue Farbe). Herausgelöst statt abgeschrieben: Zwei Definitionen von „du bist dran" wären genau der Fehler, den dieses Projekt an `nachfassen_tasks` vs. `isDueFollowUp` schon einmal bezahlt hat. Die *Bedingung* ist bewusst nicht geteilt — ein LinkedIn-Kontakt steht über seine Fälligkeit in der Arbeitsmenge, ein Termin über seinen abgeleiteten Zustand plus den Nachfass-Stempel; ein gemeinsamer Union-Typ über zwei so verschiedene Quellen wäre wieder Überbau.
- **Nachfass-Stempel** = `follow_up_last_contacted_at` + `follow_up_last_contacted_by_user_id` auf beiden Termin-Tabellen (Migration 0041, §3). Der Ein-Klick-Knopf „Genervt" schreibt hinein; die Zeile wird dadurch ruhig und leuchtet morgen wieder. **WIE genervt wurde, interessiert bewusst niemanden** — „DM, Anruf, WhatsApp, Mail, Brieftaube": kein Kanal, keine Stufe, kein Text. Die Spalte hält **einen Zeitpunkt, keine Historie**: Wer dreimal nachgefasst hat, hinterlässt eine Zeile, nicht drei. Ein Ereignis-Log je Klick wäre genau der Überbau, der hier abgeräumt wurde. Derselbe Stempel schließt auch die **Termin-Erinnerungen** (oben) — dort allerdings gegen die erreichte Marke gelesen und nicht gegen den Kalendertag: Ein Stempel von heute früh darf die Erinnerung eine Stunde vor dem Termin von heute Abend nicht mitnehmen.
...
3. **Steht ein Termin?** Dann ist er versorgt → `verlegt`. Verglichen wird auf **Berliner
   ...
   mahnen — dafür gibt es den No-Show-Eintrag. **Quer dazu, nicht darin:** Dieselbe Zeile
   kann trotz `verlegt` auf der Arbeitsliste stehen, wenn eine ihrer zwei Erinnerungen offen
   ist (Begriff „Termin-Erinnerung" oben). Der Zustand ändert sich dadurch nicht — der
   Zustandsname sagt „ein Termin steht", und das bleibt wahr. Weil `verlegt` erst hier
   entsteht, ist zugleich jeder darüber liegende Fall von den Erinnerungen ausgenommen:
   abgesagt, abgesagt ohne Aussicht, tot, unqualifiziert, gewonnen, verloren, ins Closing
   weitergeschoben. Eine zweite Prüfung derselben Fälle gibt es deshalb nicht.
...
- **Personen-Zuordnung von Terminen: `assigned_user_id`, nicht `created_by_user_id`.** `setting_calls` und `closing_calls` haben **kein** `owner_name`; stattdessen trägt jede Zeile genau EINE zuständige Person in `assigned_user_id`. Die Personenachse der Datenbank ist `coalesce(assigned_user_id, created_by_user_id)` — in SQL wörtlich so, im Code `personOf()` / `personIn()` (`src/lib/personResolution.ts`). **Seit Migration 0042 zählen die Analyse-Tabs davor noch „Durchgeführt von"** (`coalesce(conducted_by_user_id, assigned_user_id, created_by_user_id)`, `gespraechsPersonOf()`); „Termine gelegt" bleibt bei der Datenbank-Achse. Die drei Personenfragen stehen unten im Unterpunkt „Durchgeführt von".
  ...
  - **Gesetzt wird beim Anlegen, nie beim Lesen** (`src/app/actions/appointments.ts`, `createClosingFromSetting`): Erstgespräch (LinkedIn, Telefon, manuell) → **wer bucht** (`assignedUserForBooking`, seit 15. September 2026 — „wenn ich einen Termin lege, muss ich ihn an den Termin erinnern"; vorher der Owner der Quellliste), ersatzweise — wenn der Buchende kein Mitglied ist, also ein Plattform-Admin in einer Kunden-Organisation — der Owner der **Quellliste** (`owner_name` hat Vorrang vor `created_by_user_id`, `ownerUserIdOfList`); Closing → **fest an die Closing-Person** (`CLOSING_ZUSTAENDIG_USERNAME = "Simon"`, `closingZustaendigIn()` in `src/lib/personResolution.ts`, seit 15. September 2026 — „alle Closing-Termine sollen automatisch mir zugeordnet werden"), aufgelöst gegen die Mitglieder der aktiven Organisation; ist sie dort kein Mitglied (Kunden-Organisation, Plattform-Admin, s. o.), erbt das Closing wie bisher vom Setting, sonst fällt es auf den Anlegenden. Ein bereits bestehendes Closing wird dabei nicht umgehängt; Closings von vor diesem Datum tragen weiter die geerbte Zuweisung. Bestandsdaten per Backfill in 0028 nach derselben Reihenfolge (bestehende `call_assignees`-Zeile → Owner der Quellliste → Ersteller der Quellliste → Ersteller des Termins). Deshalb braucht keine Auswertung Quellketten-Joins.
  - **„Durchgeführt von" (`conducted_by_user_id`, Migration 0042) — die dritte Personenspalte, seit 15. September 2026.** Mit mehreren Settern fielen drei Fragen auseinander, die bis dahin alle an `assigned_user_id` hingen: „Wenn ich einen Termin lege, muss ich ihn erinnern; wenn er nicht erscheint, muss weiterhin ich ihn nerven — aber wenn Kevin das Closing gemacht hat, bekommt er es zugerechnet." Beide Termin-Tabellen tragen deshalb die Person, die das Gespräch **geführt** hat. Immer leer, bis jemand sie einträgt (Detailseite, **jedes** Mitglied darf, `setConductedBy` in `src/app/actions/assignees.ts`), kein Default, kein Backfill; ein Guard aus 0042 nullt eine Durchführung über die Org-Grenze. Die drei Fragen und ihre Regeln (alle in `src/lib/personResolution.ts`):
    - **Wer erinnert und nervt** (Arbeitsliste „Mein", Kalender) → `erinnererOf()`: die Zuweisung, also wer gelegt hat. **Einzige Ausnahme:** Zustand `show` (erschienen, kein Ergebnis — beim Closing auch „Nachfassen") geht an „Durchgeführt von", sofern eingetragen. No-Show und „Nicht qualifiziert" bleiben beim, der gelegt hat.
    - **Wem die Analyse zählt** (Termine, Show- und Quali-Quote, Umsatz, Abschlussrate — Übersicht-, Setting-, Closing- und Funnel-Tab, Vergleichsseite, Umsatz auf `/team`) → `gespraechsPersonOf()`: „Durchgeführt von", sonst die Zuweisung.
    - **Personenachse der Datenbank** (RLS, `rpc_appointments_booked` = „Termine gelegt", `recycle_tasks`, LinkedIn-Tab „Termine gelegt") → unverändert `personOf()` = `coalesce(assigned_user_id, created_by_user_id)`. Gebucht hat, wer gelegt hat.
    **Für SQL heißt das:** Wer die Zahlen der Analyse-Tabs nachrechnet, braucht `coalesce(conducted_by_user_id, assigned_user_id, created_by_user_id)`; „Termine gelegt" bleibt bei `coalesce(assigned_user_id, created_by_user_id)`. Die RLS-Policies `*_scoped_member` kennen die Spalte seit 0042 als dritten Zweig: Wer ein Gespräch geführt hat, sieht es auch mit `data_scope='own'`.
...
| **Ausschnitt „Zu tun · Termin steht · Alle"** | `/termine` | der abgeleitete ZUSTAND (`istZuTun`, `src/lib/dranRegel.ts`) — also genau „neu terminiert oder tot", die beiden Bedingungen aus dem Satz oben. Die offene Termin-Erinnerung ist der einzige Zusatz und schneidet in die Gegenrichtung: Sie nimmt nichts heraus, sie holt einen anstehenden Termin für die Stunden herein, in denen anzukündigen ist |
...
- **Die Arbeitsliste rechnet auf Berliner Kalendertagen, nicht auf Minuten.** Beide Fragen, aus denen sie besteht, sind Tagesfragen: „Steht ein Termin?" (`terminZustand`, Vergleich `Berlin-Tag(appointment_at) >= heute`) und „Habe ich den heute schon genervt?" (`istHeuteKontaktiert`, `berlinDateISO(stempel) === heute`). Ein Termin, der heute um 10:00 war, soll nicht ab 10:01 golden mahnen — dafür gibt es den No-Show-Eintrag. **Zwei Ausnahmen:** der Rückruf-Reiter (`callback_at`, Körnung `moment` — dort steht eine mit dem Lead verabredete Uhrzeit) und die zwei Termin-Erinnerungen (s. u.).
- **Die zwei Termin-Erinnerungen rechnen auf die Minute — und die Tages-Marke in WANDZEIT.** „1 Stunde vorher" ist eine **Dauer**: sechzig Minuten, auch am Umstellungssonntag (als Wandzeit gerechnet müsste man im Frühjahr eine Uhrzeit bilden, die es nicht gibt — 02:30). „1 Tag vorher" ist dagegen eine **Kalenderaussage**: dieselbe Uhrzeit am Vortag. An den Umstellungswochenenden ist ein Tag 23 bzw. 25 Stunden lang; eine Millisekunden-Rechnung landete dort auf 09:00 statt 10:00 — eine Uhrzeit, die niemand gemeint hat. Gerechnet wird deshalb ISO → Berliner Wandzeit → Ziffernarithmetik auf dem Tagesstring → `berlinInputToIso()` zurück (`erinnerungsZeitpunkte()`, `src/lib/dranRegel.ts`) — derselbe Weg, den die gefallene Kaskaden-Engine ging, und dieselbe Überlegung, die den Tagesvergleich der Arbeitsliste trägt.
- **Eine erreichte Erinnerungs-Marke ist NICHT „überfällig", und sie wird nie so gelesen.** Die Rechnung wäre dieselbe wie in `isOverdue(value, "moment")` — die Vokabel ist die falsche. Wer heute um 14:00 einen Termin für heute 18:00 anlegt, hat die Vortags-Marke im Moment des Buchens schon überschritten; über `isOverdue` gelesen wäre die Zeile in derselben Sekunde rot, in der sie entsteht. Genau daran ist die alte Kaskade zurückgewiesen worden. Eine Erinnerung kennt deshalb nur zwei Zustände — noch nicht dran, oder dran (Gold) —, und die Zeile beschriftet die **Lage** („Termin heute", „Termin morgen", „Termin in weniger als einer Stunde"), nicht eine verpasste Frist. Die Begründung steht als Warnblock in `src/lib/dueState.ts`, damit sie dort liegt, wo jemand die Wiederverwendung versuchen würde.
...
**Manuell auszuführende Migrationen:** `…0019`, `…0020`, `…0021`, `…0022` (pg_trgm-Suchindizes), `…0023` (`list_views`), `…0024` (Schema-Abgleich), `…0025` (`platform_admins`), `…0026` (Nutzer-Umzug), `…0027` (Organisation löschen), `…0028` (`fundament`: Zuweisung, Anruf-Log, RPC-Korrekturen), `…0029` (`analyse_umbau`: Quellen, Termin-Rufnummer, Verlustgrund-Codes, Telefon-Skripte), `…0030` (`script_label_snapshot`), `…0031` (`message_templates`), `…0032` (`reminder_cascade`), `…0033` (`lead_recycling`), `…0034` (`tenant_lifecycle`), `…0036` (`tenant_lifecycle_nachtrag`), `…0037` (`rpc_zugriffspruefung`), `…0038` (`umzug_anruflog`), `…0039` (`apply_touches_pruefung`) `…0040` (`schreibpfad_mandantengrenzen`) und `…0041` (`rueckbau_pflichtfelder_und_nachfass_stempel`) laufen nicht automatisch — sie müssen im Supabase-SQL-Editor ausgeführt werden. **Alles bis einschließlich 0041 ist eingespielt und damit eingefroren.** Der Ablauf im Einzelnen: 0031 bis 0034 am **8. September 2026** (Verifikation bestanden: Katalog 31 Einträge, je Organisation eine `pipeline_settings`-Zeile und 21 `cascade_steps`, davon 16 aktiv), 0036 und 0037 kurz darauf, **0035, 0038, 0039 und 0040 am 10. September 2026 zusammen mit dem Deploy** des Nachfassen-Umbaus auf `main`. **`…0042` (`durchgefuehrt_von`, die Spalte „Durchgeführt von" samt Guard und drittem RLS-Zweig, §2) ist geschrieben und noch NICHT eingespielt — sie muss VOR dem Deploy des zugehörigen Codes laufen**: Die Analyse selektiert die Spalte namentlich und `/termine` nennt sie bei aktiver Datensicht im Personenfilter (Muster 0029/0032); zu früh ist folgenlos. Verifikationsblock am Dateiende. **Die nächste Schema-Änderung danach braucht die Nummer 0043** — an keiner der Dateien 0031–0041 darf noch etwas geändert werden, auch nicht an einer, die „nur" eine Funktion neu schreibt: Wer sie ein zweites Mal einspielt, spielt dann etwas anderes ein, als beim ersten Mal lief. 0041 ist am **12. September 2026** eingespielt worden und damit ebenfalls eingefroren.

Dass 0036 und 0037 vor 0035 eingespielt wurden, obwohl 0035 dazwischen liegt, war **kein Versehen**: Die drei sind voneinander unabhängig (keine berührt eine Tabelle), und 0035 war die einzige, die auf den Deploy warten musste — sie hätte die damals produktive App zerbrochen (s. u.). Die Nummer ist eine Reihenfolge im Ordner, keine Zwangsfolge beim Ausführen. Für 0041 galt dasselbe mit umgekehrtem Vorzeichen: Sie musste **vor** dem Code laufen, der zu ihr gehört — so ist es auch geschehen.
...
### 0041 — eingespielt am 12. September 2026 (sie trug ein Deploy-Risiko)
...
>
> **Teil 2 musste vor dem Deploy laufen** — seit dem 12. September 2026 ist das erledigt. Der
> folgende Abschnitt beschreibt den Zustand, der eintritt, **solange die vier Spalten fehlen**.
> Er ist damit keine offene Aufgabe mehr, sondern die Beschreibung des Rückfalls: relevant für
> eine frisch aufgesetzte Datenbank und für jeden, der die Fehlermeldung im Code findet und
> wissen will, wovon sie redet.
...
-- „Durchgeführt von" (Migration 0042) zeigt nur auf Mitglieder DERSELBEN
-- Organisation — dieselbe Regel wie die Zuweisung darüber. Der Guard
-- `conducted_by_guard` greift beim INSERT, beim Setzen und bei 'update of
-- workspace_id'; ein Treffer heißt also wieder: beim Nutzer-Umzug
-- ZURÜCKGEBLIEBEN. Die Analyse fiele dort auf die Zuweisung zurück, der
-- Termin zählte aber bei niemandem, den es in der Organisation gibt.
select count(*) from setting_calls sc where sc.conducted_by_user_id is not null
  and not exists (select 1 from workspace_members wm
                   where wm.user_id = sc.conducted_by_user_id and wm.workspace_id = sc.workspace_id);
select count(*) from closing_calls cc where cc.conducted_by_user_id is not null
  and not exists (select 1 from workspace_members wm
                   where wm.user_id = cc.conducted_by_user_id and wm.workspace_id = cc.workspace_id);
